...
export const FaceRecognitionDialog = ({ open, onOpenChange }) => {
  ...
  const startFaceRecognition = async () => {
    ...
    try {
      ...
      // Step 1: Send photo to bot and get match results
      ...
      // Step 2: Auto-fetch the top match NIK details
  ...
  const generatePDF = () => {
    ...
    yPos = 280;
    pdf.setFontSize(8);
    pdf.text('Generated by WASKITA LBS - Face Recognition Module', 105, yPos, { align: 'center' });

    const fileName = `FR_${nikData?.NIK || 'result'}_${new Date().toISOString().split('T')[0]}.pdf`;
    pdf.save(fileName);
  ...
  const getStepIndicator = () => {
    const steps = [
      { key: 'upload', label: 'Upload Foto' },
      { key: 'matching', label: 'Pencocokan' },
      ...
      <div className="flex items-center justify-center gap-2 mb-4">
        {steps.map((step, idx) => (
          <React.Fragment key={step.key}>
            ...
            {idx < steps.length - 1 && (
              <div className={`w-8 h-0.5 ${idx < currentIndex ? 'bg-green-500' : 'bg-gray-600'}`} />
            )}
          </React.Fragment>
        ))}
      </div>
                    ...
                    >
                      ...
                      {uploadedImagePreview && (
                        <img 
                          src={uploadedImagePreview} 
                          alt="Input" 
                          className="max-h-40 mx-auto rounded-lg border-2"
                    ...
                    <div className="space-y-2">
                      {matchResults.map((match, idx) => (
                        ...
                        >
                          ...
                          {idx === 0 && (
                            <span className="text-xs text-green-400">✓ Top Match</span>
                          )}
                        </div>
                      ))}
                    </div>
