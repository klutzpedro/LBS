...
async def simple_query(request: SimpleQueryRequest, username: str = Depends(verify_token)):
    ...
    try:
        ...
        if query_type == 'capil_name':
            ...
            # Send NKK command
            await telegram_client.send_message(bot_entity, f"NKK {query_value}")
            await asyncio.sleep(5)
            
        elif query_type == 'reghp':
            # Send RegHP command
            await telegram_client.send_message(bot_entity, f"REGHP {query_value}")
            await asyncio.sleep(5)
            
        elif query_type == 'passport_wna':
            # Send name for passport WNA search
            await telegram_client.send_message(bot_entity, query_value)
            await asyncio.sleep(3)
            ...
            # Send name for passport WNI search
            await telegram_client.send_message(bot_entity, query_value)
            await asyncio.sleep(3)
            ...
            # Send passport number
            await telegram_client.send_message(bot_entity, f"PASS {query_value}")
            await asyncio.sleep(5)
            
        elif query_type == 'plat_mobil':
            # Send plate number
            await telegram_client.send_message(bot_entity, f"PLAT {query_value}")
            await asyncio.sleep(5)
        ...
        for msg in messages:
            ...
            if msg.text and len(msg.text) > 20:
                ...
                if not any(skip in msg.text.lower() for skip in ['pilih', 'choose', 'select']):
                    raw_response = msg.text
                    break
        
        if raw_response:
            logger.info(f"[SIMPLE QUERY] Got response: {raw_response[:100]}...")
            return {
                "success": True,
            ...
            logger.warning("[SIMPLE QUERY] No response from bot")
            return {
                "success": False,
                "query_type": query_type,
                "query_value": query_value,
                "error": "Tidak ada respons dari bot"
            }
