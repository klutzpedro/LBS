...
@api_router.get("/auth/transfer-request/{request_id}")
async def check_transfer_request_status(request_id: str):
    """Check status of device transfer request (called by new device polling)"""
    transfer_req = await get_transfer_request(request_id)
    
    if not transfer_req:
        return {"status": "not_found", "message": "Request tidak ditemukan atau sudah kedaluwarsa"}
    
    # Check if request has timed out
    created_at = datetime.fromisoformat(transfer_req["created_at"].replace('Z', '+00:00'))
    if (datetime.now(timezone.utc) - created_at).total_seconds() > TRANSFER_REQUEST_TIMEOUT:
        await delete_transfer_request(request_id)
        return {"status": "timeout", "message": "Request kedaluwarsa. Silakan coba login lagi."}
    
    status = transfer_req.get("status", "pending")
    
    if status == "approved":
        # Create session for new device
        username = transfer_req["username"]
        device_info = transfer_req["new_device_info"]
        
        # Check if admin
        is_admin = username == ADMIN_USERNAME
        
        session_id = secrets.token_hex(32)
        await create_session(username, session_id, device_info)
        ...
        await db.security_logs.insert_one({
            "ip": "transfer",
            "event_type": "login_success",
            "details": f"{'Admin' if is_admin else 'User'} login via device transfer: {username} from {device_info}",
            "timestamp": datetime.now(timezone.utc).isoformat()
        })
        ...
        await delete_transfer_request(request_id)
        return {"status": "rejected", "message": "Permintaan pindah device ditolak"}
    
    # Still pending
    return {"status": "pending", "message": "Menunggu persetujuan dari device lain..."}

@api_router.get("/auth/pending-transfer")
async def get_pending_transfer(credentials: HTTPAuthorizationCredentials = Depends(security)):
    """Get pending transfer request for current user (called by original device polling)"""
    try:
        token = credentials.credentials
        payload = jwt.decode(token, JWT_SECRET, algorithms=[JWT_ALGORITHM])
        username = payload.get("username")
        ...
        pending = await get_pending_transfer_request(username)
        ...
        if (datetime.now(timezone.utc) - created_at).total_seconds() > TRANSFER_REQUEST_TIMEOUT:
            await delete_transfer_request(pending["request_id"])
            return {"has_pending": False}
        
        return {
            "has_pending": True,
            "request_id": pending["request_id"],
            "new_device_info": pending["new_device_info"],
            "created_at": pending["created_at"]
        }
    except:
        return {"has_pending": False}

@api_router.post("/auth/transfer-response/{request_id}")
async def respond_to_transfer(request_id: str, response: dict, credentials: HTTPAuthorizationCredentials = Depends(security)):
    """Approve or reject device transfer request (called by original device)"""
    try:
        token = credentials.credentials
        payload = jwt.decode(token, JWT_SECRET, algorithms=[JWT_ALGORITHM])
        username = payload.get("username")
        ...
        if transfer_req["username"] != username:
            raise HTTPException(status_code=403, detail="Not authorized")
        
        action = response.get("action", "reject")
        
        if action == "approve":
            await update_transfer_request_status(request_id, "approved")
            # Invalidate current session (original device will be logged out)
            await invalidate_session(username)
            
            # Log
            await db.security_logs.insert_one({
                "ip": "transfer",
                "event_type": "device_transfer_approved",
                "details": f"User {username} approved device transfer to {transfer_req['new_device_info']}",
                "timestamp": datetime.now(timezone.utc).isoformat()
            })
            
            return {"success": True, "message": "Transfer disetujui. Anda akan logout dari device ini."}
        else:
            await update_transfer_request_status(request_id, "rejected")
            
            # Log
            await db.security_logs.insert_one({
                "ip": "transfer",
                "event_type": "device_transfer_rejected",
                "details": f"User {username} rejected device transfer to {transfer_req['new_device_info']}",
                "timestamp": datetime.now(timezone.utc).isoformat()
            })
            
            return {"success": True, "message": "Transfer ditolak"}
    
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Transfer response error: {e}")
